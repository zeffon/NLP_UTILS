...
BATCH_SIZE = 120
SEQ_LENGTH = 60
PRE_MODEL_DIR = "./pre_model/chinese_L-12_H-768_A-12"


def main():
    text = "余泽锋，男，汉，python工程师，2018年末开始接触数据挖掘和模式识别工作。"
    # preprocess
    tokenizer = FullTokenizer(vocab_file=PRE_MODEL_DIR+"/vocab.txt")
    tokens = []
    ...
    with graph.as_default():
        ...
        config = BertConfig.from_json_file(PRE_MODEL_DIR+"/bert_config.json")
        ...
        with tf.Session(graph=graph) as sess:
            sess.run(init)
            saver.restore(sess, PRE_MODEL_DIR+"/bert_model.ckpt")
            print(output.shape)
            # embeddings shape []
